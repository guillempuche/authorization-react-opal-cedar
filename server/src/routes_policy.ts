...
export const routesPolicy = async (router: Router): Promise<void> => {
	...
	router.get('/policy-config', policyAuthentication, (ctx) => {
		ctx.response.status = 200;
		ctx.response.headers.set('Content-Type', 'application/json');
		ctx.response.body = {
			external_source_url: `http://localhost:${env.PORT_SERVER}/policy-data`,
		};
	});
	// Route to get policy configuration
	router.get('/policy-data', policyAuthentication, async (ctx) => {
		ctx.response.status = 200;
		ctx.response.headers.set('Content-Type', 'application/json');
		// ctx.response.body = {
